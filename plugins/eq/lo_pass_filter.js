...
if (needsReset || !context.cachedCoeffs) {
  ...
  if (topologyReset) {
    ...
    if (lpSections.length > 0) {
      for (let j = 0; j < lpSections.length; j++) {
    ...
    if (queueFrequencyChange) {
      ...
      context.pendingCoeffs = lpSections;
    } else {
      context.transitionCoeffs = lpSections;
...
// Apply Linkwitz-Riley low-pass filter. Frequency-only changes render a
...
class LoPassFilterPlugin extends PluginBase {
  ...
  createUI() {
    ...
    const slopeSelect = createSlopeSelect(this.sl, v => this.setSlope(v), "LPF", canvas);
