...
class DSD64IMDSimulatorPlugin extends PluginBase {
    ...
    createDryWetControl() {
        ...
        // `dw` is automatable, so this hand-built row has to follow changes pushed
        // from outside the UI (host automation, preset recall). The slider maps the
        // model straight through, exactly as the initial render above does, and the
        // readout is regenerated by the same formatDryWet(). Only the slider is
        // tested for held-ness: valueEl is a read-only mirror the user cannot edit,
        // so treating a click that focuses it as an edit in progress would freeze
        // the row instead of protecting anything.
        this.registerUIRefresh(() => {
            if (this.isHeldByUser(slider)) return;
            slider.value = this.dw;
    ...
    createUI() {
        ...
        // Main parameters
        c.appendChild(this.createParameterControl('Amount', -40.0, 50.0, 0.1, this.am, v => this.setAm(v), 'dB', 'am'));
        c.appendChild(this.createDryWetControl());
        c.appendChild(this.createParameterControl('Ultrasonic Level', -48.0, -18.0, 0.5, this.ul, v => this.setUl(v), 'dBFS RMS', 'ul'));
        c.appendChild(this.createParameterControl('Noise Color', -100, 100, 1, this.nc, v => this.setNc(v), '%', 'nc'));
        c.appendChild(this.createParameterControl('Analog Nonlinearity', 0.0, 10.0, 0.01, this.an, v => this.setAn(v), '%', 'an'));
        c.appendChild(this.createParameterControl('Even Bias', 0, 100, 1, this.eb, v => this.setEb(v), '%', 'eb'));
        c.appendChild(this.createParameterControl('Signal Coupling', 0, 200, 1, this.sc, v => this.setSc(v), '%', 'sc'));
        c.appendChild(this.createParameterControl('IMD Path HPF', 0.0, 8.0, 0.1, this.hf, v => this.setHf(v), 'kHz', 'hf'));
        c.appendChild(this.createParameterControl('Scratch Tone', 3.0, 14.0, 0.1, this.st, v => this.setSt(v), 'kHz', 'st'));

        // Advanced / utility parameters
        c.appendChild(this.createParameterControl('Noise Texture', 0, 100, 1, this.nt, v => this.setNt(v), '%', 'nt'));
        c.appendChild(this.createParameterControl('Cross Sideband', 0, 100, 1, this.cs, v => this.setCs(v), '%', 'cs'));
        c.appendChild(this.createParameterControl('Output Trim', -24.0, 12.0, 0.1, this.ot, v => this.setOt(v), 'dB', 'ot'));
