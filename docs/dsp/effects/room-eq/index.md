...
- **Low-frequency Phase Extension** is off by default, preserving the established Correction behavior and the sound of existing presets. Enable it only when you want excess-phase correction below Phase Low. Lower frequencies use progressively longer analysis windows and can include later room response, so Consensus is the safest starting point. Compare more than one listening position and turn the extension off if bass timing becomes less consistent.
...
- **Low-frequency Phase Extension** - Extends measured excess-phase correction from Phase Low toward Correction Low by using progressively longer, frequency-dependent analysis windows below Phase Low. Phase Low and higher frequencies keep the established fixed-window correction. This setting is off by default, and presets that do not contain it remain off. It is available only in Correction; the control is disabled in Minimum and Linear while its selected value is retained. If the measured impulse response is too short for a requested low-frequency window, Room EQ uses the available shorter measurement window and shows a warning. It reduces or skips the correction only when the realized FIR approaches its time limits or the filter-design calculation budget is exceeded; the rest of the Room EQ filter remains active.
...
- Use the **Frequency**, **Phase**, **Group Delay**, and **Impulse** radio buttons at the top of the graph to switch views.
