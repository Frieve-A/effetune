...
| What do Silence threshold and Stop audio input after change? | **Silence threshold** (-90/-80/-70/-60 dBFS) controls the measured input/output power below which EffeTune treats audio as silent; a lower value is less likely to classify quiet audio as silence. In **Maximum power saving**, **Stop audio input after** (1/5/15 minutes or **Never**) controls only microphone/input release. It is independent of the shorter delay used to suspend an idle no-route audio graph, so the graph may suspend while the input is still retained. |
