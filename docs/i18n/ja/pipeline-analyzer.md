...
- **シーケンス長** は、MLSまたはTSPで長い応答をどこまで正しく測れるかを決めます。長くすると時間とメモリを多く使います。Delay、Reverbなど余韻の長いエフェクトでは、特に推奨値が表示された場合に長くしてください。
...
詳細には、現在の長さが十分か、推奨シーケンス長と安定化時間、測定にかかる合計時間が表示されます。推奨値は、測定するエフェクトに合う場合に設定へ反映してください。

シーケンス長、安定化周期数、平均回数が無効になるのはUnit Impulseだけです。Frequency、Phase、Min Group Delay、Excess Group Delay、Impulseの切り替えは表示だけを変更し、再測定しません。
...
一般的な特性測定にはMLSまたはTSPが適しています。Delay、Reverb、リンギングなどが現在の測定範囲を超えると、応答が重なって表示されることがあるため、**シーケンス長**を長くしてください。**Unit Impulse**は応答を一定時間だけ直接収録するため、非常に長い余韻は途中で切れることがあります。
