...
- [Phase Select EQ](#phase-select-eq) - L/R位相差で選んだ周波数成分をブーストまたはカット
...
Phase Select EQは、左右チャンネル間の絶対位相差を条件に、ステレオ信号の特定の周波数成分だけをブーストまたはカットします。左右のスペクトルには同じ正のゲインを適用するため、位相差を回転、補正したり、新しい位相差を作ったりするエフェクトではありません。通常の周波数だけを基準にするEQでは分けにくい成分が、特定の周波数とステレオ位相の範囲に現れる場合に使用します。
...
- 横軸は表示用の符号付きL/R位相差です。中央の0°は同相を表し、左右端の-180°と+180°は同じ逆相の点です。
...
- 選択に使うのは位相差の**絶対値**です。そのため1つの論理領域は0°を中心に鏡像表示され、+60°と-60°の成分を同じように処理します。L/Rを入れ替えるとドットは左右反転しますが、処理対象は変わりません。
- 内側の実線枠がCore、外側の破線枠がTransitionです。0°を含むBandは中央でつながり、180°まで達するBandはマップの左右端をまたいで続きます。
...
マップ上のハンドル、スライダー、数値入力は同じ値を編集します。選択中のBandは外枠内のどこをドラッグしてもBand全体が移動し、Coreの辺または隅をドラッグするとサイズが変わります。外側の辺にあるハンドルでは各Transitionを個別に変更できます。低位相側のハンドルは中央を越えず、Core Low Phaseは0°、Low Phase Transitionは最大幅で停止します。Core Low Phaseがちょうど0°のときは中央のハンドルを最初に左右どちらへも動かせますが、いったん片側へ動かすと、そのドラッグが終わるまで同じ側に固定されます。
