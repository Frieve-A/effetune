...
- [DSD64 IMD Simulator](#dsd64-imd-simulator) - DSD64の超音波ノイズに由来する可聴域の相互変調歪み（IMD）をシミュレート
...
DSD64再生でしばしば議論される副作用を再現するエフェクトです。DSDが可聴帯域より上に持つ超音波ノイズが、DAC・アンプ・スピーカーなどのわずかな粗（非線形性）によって相互変調歪み（IMD）を発生し、その一部が可聴帯域に落ちてくる現象を再現します。通常は聞こえないこの音を、可聴かつ調整可能な形で再現するシミュレーションで、実際のDSD信号は生成しません。

**このエフェクトは88.2kHz以上のサンプルレートが必要です**（88.2 / 96 / 176.4 / 192kHz）。44.1 / 48kHzでは動作できないためバイパスされ（ドライ信号がそのまま通過）、警告が表示されます。このエフェクトを使うには、アプリのオーディオ設定でサンプルレートを88.2kHz以上に設定してください。

### サウンドキャラクターガイド
- 非常に控えめな「デジタルのざらつき」:微かで一定の砂状のノイズフロアに、音楽に追従する細かい硬さが加わります。
- デモンストレーション用途:通常は聞こえないDSD64の超音波IMDを、可聴かつ可変な形で確認できます。
- クリエイティブなテクスチャ:Amount や Analog Nonlinearity を上げると、明確なローファイの擦れ／エッジ効果になります。

### パラメータ

メインパラメータ
- **Amount**（-40.0 〜 +50.0 dB）- 生成される歪み全体のレベル。
- **Dry-Wet**（100:0 〜 0:100）- ドライ信号と生成された歪みのバランス。`ドライ:ウェット` 比で表示します。100:0 = ドライのみ、100:100（中央）= フルドライ＋フル歪み、0:100 = 歪みのみ。
- **Ultrasonic Level**（-48.0 〜 -18.0 dBFS RMS）- シミュレートするDSD超音波ノイズのレベル。ノイズが多いほど歪みが増えます。
- **Noise Color**（-100 〜 +100%）- 超音波ノイズを低め/高めの周波数へ移動させ、バランス（傾き）を調整します。
- **Analog Nonlinearity**（0.00 〜 10.00%）- シミュレートするアナログ機器の粗さ（非線形性）。高いほど歪みが増えます。
- **Even Bias**（0 〜 100%）- 歪みの構成バランスを調整します。低いほど音楽に追従する歪み（Attached）が中心になり、高いほど入力に依らない一定の歪み（Additive）とクロス成分（Cross）が中心になります。
- **Signal Coupling**（0 〜 200%）- 音楽に依存する歪み（Attached と Cross）の強さ。0 にすると一定の Additive ノイズだけが残ります。
- **IMD Path HPF**（0.0 〜 8.0 kHz）- 歪みの生成をこの周波数より上に限定します。0.0 = Off（フルレンジ、アンプのような挙動）。約2.5kHzで「ツイーターのみが歪みを発生する」システムを再現します。ドライ信号には一切影響しません。
- **Scratch Tone**（3.0 〜 14.0 kHz）- 可聴「擦れ」成分の中心周波数。

詳細／ユーティリティパラメータ
- **Noise Texture**（0 〜 100%）- 超音波ノイズに共振リップルを加え、質感をわずかに変えます。
- **Cross Sideband**（0 〜 100%）- 音楽と超音波ノイズが混ざって生じる歪みの量。
- **Output Trim**（-24.0 〜 +12.0 dB）- 最終出力レベルの調整。

### 可視化
- **Term Contribution メーター** - エフェクトの各要素のリアルタイムレベル:
  - **Additive** - 入力が無くても存在する、ノイズ由来の一定の歪み。
  - **Attached** - 音楽に貼り付いて追従する歪み。
  - **Cross** - 音楽と超音波ノイズが混ざって生じる歪み。
  - **Total IMD** - 生成された歪みの合計。
  - **Output** - 最終出力レベル（Dry-Wet と Output Trim 適用後の、ドライ＋歪み）。
- **Analog Transfer Curve** - Analog Nonlinearity と Even Bias が作る歪みカーブを、Saturation系プラグインと同じ in/out 形式で表示します。
- **Difference-Frequency 表示** - 現在のノイズ設定に基づき、超音波ノイズがどの可聴周波数を生成するかを示す静的なグラフ。
...
- 効果を強調:Amount、Ultrasonic Level、Analog Nonlinearity を上げます。
