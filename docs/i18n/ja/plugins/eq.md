...
Room EQは、EffeTuneに保存した周波数応答測定からFIR補正フィルターを作成します。初期状態では、共有の測定から1つのフィルターを設計し、プラグインを通るすべてのチャンネルに適用します。個々のチャンネルに別の測定を割り当てると、そのチャンネルだけ専用のフィルターになり、それ以外の設定はインスタンス全体で共通のままです。対象チャンネルは、プラグイン共通のバス選択で指定します。選択した測定の全ポイントを平均し、応答を平滑化して、指定した補正範囲内の偏りを抑えます。スピーカーと部屋の相互作用により、リスニング範囲で繰り返し現れるピークや広帯域の音色の偏りを整えたい場合に使用します。リニアフェーズの振幅補正、またはミニマムフェーズの振幅補正と測定した余剰位相の補正を組み合わせたミックスドフェーズ補正にも対応します。余剰位相の補正では、Phase Correctionが直接音を、Reverb Correctionがそれに続く室内残響を対象にします。初期状態では、どちらの補正も各測定ポイントに共通する成分を残し、ポイント間で一致しない部分の補正を弱めます。Room EQにはWASM DSPエンジンが必要です。利用できない場合、信号は変更されずに通過します。
...
- **Low-frequency Phase Extension** - Phase Lowより低い周波数で、低域ほど長い周波数依存の時間窓を使い、測定した余剰位相の補正をCorrection Lowへ向かって広げます。Phase Low以上では固定長の時間窓で補正します。初期状態はオフです。Correctionでのみ使用でき、MinimumとLinearでは選択内容を保持したまま操作できなくなります。測定したインパルス応答が要求された低域の時間窓より短い場合は、利用可能な短い測定時間窓を使って補正を試み、警告を表示します。実際に生成したFIRが時間方向の余裕に近づく場合、またはフィルター設計の計算量が上限を超える場合に限り、補正量を減らすか補正を省略します。それ以外のRoom EQフィルターは引き続き有効です。この拡張はPhase Correctionが0%より大きい間だけ機能し、0%ではReverb Correctionを併用していても動作しません。
...
- **Reverb Correction** - 測定した室内残響の補正を0～100%の範囲で1%刻みに調整し、Correctionでのみ機能します。MinimumとLinearでは設定値を保持したまま操作できなくなります。初期値の0%では残響補正は完全にオフで、Room EQはこのコントロールがない場合とまったく同じ動作になります。0%より大きくすると、Reverb Windowの範囲で応答を解析し、Reverb Max Freqまでの帯域で直接音の後に続く残響を打ち消します。残響の余剰位相をPhase Correctionとは独立して補正するとともに、同じ帯域にReverb Smoothingに従うより細かな振幅補正を加え、残響によるコム状のうねりや減衰の遅い部屋のモードを抑えます。この細かな振幅補正もMax Boostの制限内に収まり、他の自動振幅補正と同様にLevel Correctionで比例します。直接音の位相補正が届かないPhase Lowより低い帯域では、純粋な追加補正として働きます。複数ポイントの測定では、ポイント間で一致しない部分の補正が自動的に弱まるため、初期設定ではリスニング範囲で自然な結果になります。1ポイントだけの測定ではこの安全機構が働かないため、補正は測定位置専用になります。利用できる残響の時間窓または周波数帯域が小さすぎる場合、または合成した残響補正が実際に生成したFIRの時間方向の余裕に安全に収まらない場合は、残響補正を減らすか省略して警告を表示します。それ以外のフィルターは引き続き有効です。
- **Reverb Window** - 直接音の立ち上がり後の測定応答をどこまで残響補正の解析と打ち消しに使うかを20～1000 msで指定します。長くするほど、より後まで続く残響に対応します。実効の残響時間窓は、FIRのタップ数、Direct Window、測定したインパルス応答の立ち上がり後の長さによって自動的に制限されるため、表示値より短くなることがあります。初期値の32768タップ、96 kHzでは上限は約170 msです。残響の微細構造を追従できる上限周波数は時間窓の深さに反比例します。目安はf < 1/(0.0139 × 実効時間窓)で、実効170.7 msでは約420 Hz、300 msでは約240 Hzです。タップ数による制限、短い測定インパルス応答、Direct Window以下のReverb Window設定のいずれかにより実効時間窓がDirect Windowを超えない場合、残響補正は全体が省略され、警告が表示されます。それ以外のフィルターは引き続き有効です。
...
- **Reverb Smoothing** - 残響補正が位相解析とより細かな振幅補正の両方で使う、0.02～1.00オクターブのガウス平滑化です。初期値の0.05 octはメインのSmoothingよりはるかに細かく、残響が応答に作る狭いうねりまで補正が追従できます。値を大きくすると残響補正は広く穏やかになります。複数ポイントの測定では、ポイント間の一致を判定する周波数スケールとしても使われます。
- **Reference Point** - Correctionで直接音と残響の両方の解析に使う余剰位相をどこから得るか選びます。初期値およびフォールバックの **合意成分（全ポイント）** では、各ポイントを時間整列して余剰位相を合成し、深いディップ付近の信頼性が低い位相の重みを下げたうえで、ポイント間の位相が一致しない部分ほど補正を弱めます。ポイント名を選ぶと、その1点の余剰位相だけを使います。振幅補正には常に全ポイントが使われます。選択したポイントを後で削除した場合は、合意成分へ戻ります。
...
- **Min Group Delay** では、振幅特性から求めた最小位相成分の遅延を表示します。**Excess Group Delay** では、その最小位相成分を差し引いた残りの遅延を独立して表示するため、反射などによる非最小位相の時間変化を確認しやすくなります。どちらも横軸は対数周波数、縦軸はミリ秒です。それぞれの線は自分自身の1kHzでの値を基準にするため、どちらも1kHzで0 msとなり、正の値はその周波数が1kHzより遅れていることを表します。グレー線が補正前、緑線が実際のFIR適用後を計算した結果です。傾きは隣り合う表示点の間で求め、平滑化はSmoothingで設定した1回だけを適用します。測定した周波数特性の表示と同じ処理で、それ以外の平滑化は行わないため、Smoothingを小さくするほど細かい変化まで見えます。縦軸の範囲は表示中の曲線に合わせて自動的に切り替わります。インパルス応答データのない測定では、利用できないことを示すメッセージが表示されます。
