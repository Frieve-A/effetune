...
description: "Level Meter、Note Spectrogram、Oscilloscope、Spectrogram、Spectrum Analyzer、Stereo Meter などの音声解析プラグイン。"
lang: ja
---
...
- 色はモデルの確信度に応じて0（背景色）から1（表示色）まで変化し、弱い候補もしきい値で除外せずに表示します。確信度はモデルがその音高の存在をどの程度支持するかを示し、音量レベルや校正済みの確率ではありません。
...
- **Pitch Resolution** - 既存の履歴を消さずに、音高方向の表示解像度を切り替えます。
  - **1/12 Octave**：半音ごとに1行とし、その音の範囲で最も強い推定結果を表示します。
  - **High (1/60 Octave)**（初期値）：半音を5行に分け、細かな音高の動きを表示します。
...
- **Time Span**（1～10 s）- ピアノロールに表示する時間の長さを設定します
  ...
  - 初期値：2 s
- **Lowest Note** - 表示する音域の下限を設定します。初期値：E1。
- **Highest Note** - 表示する音域の上限を設定します。初期値：G6。
...
- **Frequency Scale** - **Log** は低域を広く表示し、**Linear** は等しい周波数幅を等間隔で表示します。
