...
- [Multiband Expander](#multiband-expander) - 周波数帯域別のダイナミックレンジ拡張を実現するプロフェッショナルな5バンドエクスパンダー
...
オーディオを5つの周波数帯域に分割し、それぞれに独立したエクスパンション処理を適用するプロフェッショナルグレードのダイナミクスプロセッサーです。特定の周波数帯域のダイナミックレンジを拡張し、閾値以下の静かな音をより静かにしながら、大きな音はそのまま維持します。マスタリング過程で過度に圧縮された音源の自然なダイナミクスを復元したり、周波数帯域別により劇的なダイナミックコントラストを作成するのに特に効果的です。

### 主な特徴
- 調整可能なクロスオーバー周波数を持つ5バンド処理
- 各バンドの独立したエクスパンション制御
- 1/fエネルギー分布に基づいた最適化されたデフォルト設定
- バンドごとのゲイン増加のリアルタイム可視化
- 高品質なLinkwitz-Rileyクロスオーバーフィルター

### リスニング向上ガイド
- **ポップス/ロック音楽:**
  - 過圧縮されたマスタリングの「壁のような」サウンドを緩和
  - ヴァースとコーラス間のダイナミックコントラストを復元
  - ストリーミング音源の平坦な印象を改善
- **クラシック音楽:**
  - 録音の自然なダイナミクスの揺らぎを復元
  - 静かなパッセージと大きなクレッシェンドのコントラストを強化
  - オーケストラ演奏本来の生き生きとした表現を取り戻す
- **ジャズ音楽:**
  - 楽器間の自然なダイナミクスを強化
  - 静かなソロをより親密に、大きなセクションをより力強く
  - 演奏の自然な呼吸感を取り戻す

### 周波数バンド
- バンド1(低域): 100 Hz以下
  - 深い低音とサブ周波数を制御
  - 自然な低音ダイナミクスのための緩やかなエクスパンションと長めのアタック/リリース
- バンド2(低中域): 100-500 Hz
  - 上部の低音と下部の中域を処理
  - 温かみと厚みを復元するための適度なエクスパンション
- バンド3(中域): 500-2000 Hz
  - 重要なボーカルと楽器のプレゼンス帯域
  - 自然さを保つためのバランスの取れたエクスパンション
- バンド4(高中域): 2000-8000 Hz
  - プレゼンスと空気感を制御
  - より速いレスポンスでの軽いエクスパンション
- バンド5(高域): 8000 Hz以上
  - 明るさとスパークルを管理
  - より穏やかなエクスパンションでの素早いレスポンスタイム

### パラメータ(バンドごと)
- **Threshold** (-60dBから0dB)
  - エクスパンションが開始されるレベルを設定
  - このレベル以下の信号が拡張（より静かに）される
- **Ratio** (1:0.05から1:20)
  - 適用されるエクスパンション量を制御
  - より劇的なダイナミックレンジ拡張には高いレシオ
- **Attack** (0.1msから100ms)
  - エクスパンションの反応速度
  - 精密なトランジェント制御には速い時間
- **Release** (10msから1000ms)
  - ゲインが通常に戻る速度
  - よりスムーズで自然なサウンドには長い時間
- **Knee** (0dBから12dB)
  - エクスパンション開始の滑らかさ
  - より自然な遷移には高い値
- **Gain** (-12dBから+12dB)
...
### ダイナミックレンジの復元
Multiband Expanderには、過圧縮された素材のダイナミクスを復元するための自然な1/fエネルギー分布に基づいた最適化されたデフォルト設定が用意されています:

- 低域バンド(< 100 Hz)
  - 制御された低音ダイナミクスのための緩やかなエクスパンション(1.2:1)
  - パンチを維持するための長めのアタック/リリース
  - 一般的な低音エネルギーに対応した閾値
...
この設定は自然なダイナミクス復元を実現します:
- すべての周波数で復元された自然なダイナミクス
- 静かなパッセージと大きなパッセージのコントラスト強化
- 最適な結果のための周波数帯域別制御
- アーティファクトのない自然で音楽的なエクスパンション
- 明瞭さと分離感の向上
- 過圧縮された録音の平坦さを軽減

### 視覚的フィードバック
- 各バンドのインタラクティブな伝達関数グラフ
- エクスパンション動作を示すリアルタイムゲインブーストメーター
- 周波数バンドのアクティビティ可視化
- 明確なクロスオーバーポイントインジケーター

### 使用上のヒント
- 一般的なダイナミクス復元にはデフォルト設定から開始
- 素材に合わせてクロスオーバー周波数を調整
- 周波数コンテンツに基づいて各バンドの閾値を微調整
- 知覚される音量変化を補正するためにゲインコントロールを使用
- 適切なエクスパンションを確保するためにゲインブーストメーターを監視
