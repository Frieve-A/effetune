...
取り込んだIRファイルは**Impulse Response Library**に保存され、元のファイル名で検索、読み込み、削除ができます。Web版ではブラウザ内に保存され、サイトデータを削除したりブラウザが保存領域を整理したりすると失われることがあります。デスクトップ版ではアプリのデータとして保存されます。必要なIRは別途保管してください。

共有URLとプリセットはIRを識別する情報だけを含み、音声データ自体は含みません。IRを利用できない場合はwet音が出ないため、同じIRを再度取り込むか選び直してください。別のIRに置き換えることもできます。dry音は**Dry**設定に従います。
