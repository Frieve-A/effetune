...
取り込んだIRファイルは**Impulse Response Library**に保存され、元のファイル名で検索、読み込み、削除ができます。WAVデータを格納した拡張子`.irs`のファイルは、名前を変更せずそのまま取り込めます。Web版ではブラウザ内に保存され、サイトデータを削除したりブラウザが保存領域を整理したりすると失われることがあります。デスクトップ版ではアプリのデータとして保存されます。必要なIRは別途保管してください。
