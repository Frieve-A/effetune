...
**設定 > 環境設定...** を開くと、言語、起動時の表示、起動時のエフェクトパイプライン動作を選べます。起動時の表示は **Effect Pipeline**（デフォルト）または **Music Library** から選択できます。Music Libraryを選んだ場合は、隣のリストで最初に表示する項目を **曲**、**アルバム**、**アーティスト**、**ジャンル**、**サブフォルダ** から選べます。
...
PCのChromium系ブラウザでは、File System Accessのフォルダハンドルを永続化できます。Safari、Firefox、モバイルブラウザなど、このAPIを利用できない環境では、選択した `File` は現在のページセッションでのみ保持されます。リロード後は毎回フォルダまたはファイルを選び直してください。EffeTuneは正規化した相対パスを使って、既存のカタログへ再接続します。

EffeTune 2.1.0では、カタログスキーマv3を採用し、CUE形式のアルバムを曲単位で閲覧できるMusic Libraryが導入されます。以前のMusic Libraryの状態は引き継がれないため、音楽フォルダを追加し直して再スキャンし、Music Libraryのプレイリストも作り直すか再インポートしてください。以前のカタログと音楽ファイル自体は変更されません。カタログは大規模なコレクションをディスクからページ単位で扱えるよう設計されています。大規模パフォーマンス測定は任意で実行するローカル開発用の診断であり、commit、release、`verify`、GitHub Actionsの合格条件でも、一般的な性能保証でもありません。
