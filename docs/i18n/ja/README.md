...
2. **音楽フォルダを追加** を選び、音楽ファイルが入っているフォルダをインデックス化します。
...
EffeTune 2.1.0では新しいMusic Libraryから始まります。以前のLibraryの状態は引き継がれないため、音楽フォルダを追加して再スキャンしてください。フォルダ内の音楽ファイル自体は変更されません。カタログは大規模なコレクションをディスクからページ単位で扱えるよう設計されています。大規模パフォーマンス測定は任意で実行するローカル開発用の診断であり、commit、release、`verify`、GitHub Actionsの合格条件でも、一般的な性能保証でもありません。
