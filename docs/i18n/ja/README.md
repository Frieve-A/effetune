...
EffeTune 3.0.0では新しいMusic Libraryから始まります。以前のLibraryの状態は引き継がれないため、音楽フォルダを追加して再スキャンしてください。フォルダ内の音楽ファイル自体は変更されません。新しい構成は100万曲を超えるカタログを設計対象としていますが、100万曲への正式対応はPhase 0の基準とランタイムを含むすべての認定に合格した後に公表します。
