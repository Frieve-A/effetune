...
- ローカルフォルダ、メタデータ、プレイリストを閲覧できるMusic Library
...
**設定** メニューの **Config** を開くと、言語、起動時の表示、起動時のエフェクトパイプライン動作を選べます。起動時の表示は **Effect Pipeline**（デフォルト）または **Music Library** から選択できます。
...
3. 曲、アルバム、アーティスト、ジャンル、フォルダ、最近追加した曲、プレイリストから閲覧し、**ライブラリを検索** でカタログ全体を検索できます。
