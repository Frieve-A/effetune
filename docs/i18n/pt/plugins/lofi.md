...
SW Radio Simulator passa a música por uma cadeia modelada de transmissão em onda curta: processamento do transmissor e modulação AM, propagação ionosférica com desvanecimento seletivo profundo, estática atmosférica e uma estação que divide o mesmo canal, um receptor de comunicações de banda estreita com detecção de envoltória ou síncrona e AGC, e um alto-falante de rádio opcional. Use-o para ouvir a música como uma transmissão internacional distante chega a um receptor de onda curta: estreita e oca, subindo e descendo com a ionosfera, assobiando onde outro transmissor está próximo em frequência.
...
- **SW Radio Simulator** modela a onda curta, em que o sinal chega por reflexão ionosférica. O desvanecimento seletivo profundo é o protagonista, a banda de áudio é mais estreita e o assobio de heteródino de uma estação no mesmo canal faz parte do som. A radiodifusão em onda curta é mono, portanto o sinal processado é sempre mono.
...
- **Tuning** (-5.0 a +5.0 kHz) - Desloca o receptor em relação à estação. Desvios pequenos abafam o som, acrescentam distorção de filtragem assimétrica e mudam o volume do assobio de heteródino; desvios maiores empurram a estação para fora da estreita banda passante de FI.
- **IF Bandwidth** (2.0 a 10.0 kHz) - Define a banda passante de FI do receptor. Ajustes estreitos correspondem à resposta de um receptor de comunicações, que rejeita mais ruído e mais da estação interferente, mas remove mais agudos; ajustes largos preservam mais detalhes e mais interferência.
- **Detector** (Envelope ou Synchronous) - Envelope é o detector a diodo comum, e é ele que transforma um desvanecimento seletivo profundo em distorção aquosa. Synchronous recupera a portadora com um PLL e demodula em relação a ela, o que reduz muito essa distorção enquanto o desvanecimento é profundo. Ele engata em cerca de ±1 kHz de Tuning e perde o engate além disso, então use Envelope enquanto move o dial. Trocar de detector reinicia a aquisição da portadora.
- **AGC Speed** (Slow, Mid ou Fast) - Define a rapidez com que o controle automático de ganho acompanha os desvanecimentos. Slow mantém as variações de nível audíveis e bombeia quando o sinal se recupera; Fast segura o nível com mais firmeza.
- **Detector RC** (20 a 500 µs) - Define o tempo de descarga do detector de envoltória. Valores longos suavizam mais a envoltória, mas aumentam a distorção de corte diagonal nos agudos com modulação forte. Não tem efeito quando Detector está em Synchronous.
...
- **MOD / EVENTS** mostra a porcentagem efetiva de modulação e, em seguida, as taxas recentes por segundo de estática (⚡) e de corte (▲), piscando quando esses eventos ocorrem. Se você quiser um resultado mais limpo e o corte for frequente, reduza Mod Depth ou Detector RC.
...
O efeito processa o primeiro par estéreo como uma única transmissão mono, assim como faz a radiodifusão real em onda curta, e o sinal recebido é sempre mono. É modelada apenas uma estação no mesmo canal, e seu programa é ruído conformado, não fala ou música. Condições reais de banda — mudanças de propagação entre dia e noite, faixas específicas de radiodifusão e recepção em banda lateral única (SSB) — estão fora deste modelo; ajuste as condições desejadas com Signal, Fading e os demais controles de propagação.
...
- **Tape Artifacts** transforma a própria música. A compressão suave, o calor acrescentado, os agudos abrandados e a instabilidade de afinação vêm todos da mesma cadeia de gravação e reprodução, portanto respondem em conjunto a Speed, Tape, Bias e Input Peak.
...
- **Compressão suave de nível:** quanto mais baixo você declarar Input Peak, mais a fita arredonda os picos antes de distorcer de forma audível, então as passagens fortes ficam mais densas e estáveis em vez de visivelmente ceifadas. No valor padrão de -6,0 dBFS e com o Bias de referência de 0,0 dB a fita tira apenas alguns centésimos de decibel dos picos de material sustentado, e até cerca de 0,8 dB dos transientes secos e do ruído. Essa quantidade cresce suavemente conforme você declara um pico mais baixo, chegando a 0,4 dB em -15,0 dBFS com um tom no pico do material. Qualquer mudança de nível maior que você observe no valor padrão vem da mudança de timbre, não da compressão, e essa vai para os dois lados conforme o material: música com muito grave pode sair cerca de 1 dB mais alta e material com muito agudo, cerca de 1 dB mais baixa.
- **Calor:** a saturação é assimétrica, portanto produz harmônicos pares e ímpares, e o calor cresce gradualmente conforme Input Peak desce, em vez de surgir de repente.
...
- **Input Peak** (-36,0 a 0,0 dBFS) - Informa à fita o quão alto é o material. Declare onde a sua música tem os picos e o modelo coloca esse pico no ponto de operação da fita, que corresponde a um pico de -18,0 dBFS. Este controle não aplica ganho nenhum por si: enquanto a fita não saturar, o mesmo sinal sai no mesmo nível em qualquer posição. Esse nível não é exatamente a unidade: fica dentro de 0,1 dB dela, um pouco acima a 30 ips e um pouco abaixo a 7,5 ips, mas não se move com Input Peak. O padrão de -6,0 dBFS serve para um master moderno comum; se você não sabe onde o material tem os picos, deixe assim e ajuste de ouvido. Os números daqui e os ajustes abaixo pressupõem música com picos em torno de -6,0 dBFS; se o seu material tiver picos em outro ponto, desloque Input Peak pela mesma diferença e os demais valores continuam válidos. Declarar um pico mais baixo do que o material realmente tem ataca a fita com essa mesma diferença a mais, e é assim que se obtêm a compressão e o calor da fita: um tom de 1 kHz no pico do material distorce 0,12% no padrão, 2% em -18,0 dBFS e 6,8% em -24,0 dBFS, enquanto em música típica os picos caem 2,7 dB nesse ajuste e 12 dB em -36,0 dBFS. Essas quedas são a fita achatando os picos, não o controle abaixando algo, então quanto mais forte a fita for atacada mais baixo fica o resultado, e Output está aí para recuperar o volume.
...
1. **Fita master de estúdio (padrão)**
   ...
   - O som de fita do dia a dia, e também o padrão do próprio plugin: o topo suavizado em 3,5 dB a 16 kHz, um reforço de 0,8 dB em torno de 80 Hz, 0,12% de distorção nos picos, um piso de chiado em -83,5 dBFS e 0,160% de wow e flutter, audível em notas longas e não nos transientes.
...
2. **Cópia limpa em alta velocidade**
   ...
   - Muito próximo do original: 0,015% de distorção nos picos, 2,2 dB a menos em 16 kHz, um fundo de -93,0 dBFS - no que se torna o ajuste Base de -89,5 dBFS a 30 ips em fita Master, com Output em 0,0 dB - e 0,053% de wow e flutter. Declarar o pico 6 dB mais alto do que ele realmente é tira 6 dB da força com que a fita é atacada, e é isso que a mantém tão limpa. Útil como referência ao comparar os demais ajustes.
...
3. **Quente e comprimido**
   ...
   - Input Peak é declarado 18 dB abaixo do pico real, então a fita é atacada 18 dB mais forte que o nominal, cerca de 4 dB além do ponto em que ela atinge 3% de distorção de terceiro harmônico. Os picos caem 2,7 dB e o fator de crista 2,4 dB em material típico - os picos de 1,6 a 4,4 dB conforme o material -, de modo que a mistura fica mais densa e quente, com 6,8% de distorção nos picos. Output sobe um pouco, em vez de descer, porque a compressão custa volume; ajuste de ouvido.
...
4. **Deck doméstico a 7,5 ips**
   ...
   - Mais escuro (9,8 dB a menos em 16 kHz, com um reforço de 1,8 dB em 50 Hz, ambos medidos depois dos seus +0,5 dB de Output), mais ruidoso (um fundo de -81,5 dBFS, que é o piso de fita de -82,0 dBFS somado a esse mesmo Output) e menos estável (0,450% de wow e flutter), com 1,4% de distorção nos picos. A polarização está um pouco alta, como costuma estar em um deck doméstico usando fita genérica: uma máquina comum em vez de uma de estúdio.

5. **Transporte gasto**
   - Speed: 7,5 ips, Tape: Standard, Bias: -2,0 dB, Input Peak: -28,0 dBFS
   - Wow/Flutter: 0,480%, Hiss: -77,5 dBFS, Output: +1,5 dB, Mix: 100%
   - 0,720% de wow e flutter, 15% de distorção nos picos, um fundo de -77,5 dBFS - o piso de fita de -79,0 dBFS somado aos seus +1,5 dB de Output - e o topo áspero e adiantado de uma máquina subpolarizada. Os picos caem 5,5 dB em material típico, e até 9 dB conforme o material, então Output precisa subir para trazer o volume de volta. Um efeito lo-fi propositalmente degradado.
