...
- **Low-frequency Phase Extension** - Estende a correção do excesso de fase medido de Phase Low em direção a Correction Low por meio de janelas de análise dependentes da frequência e progressivamente mais longas abaixo de Phase Low. Em Phase Low e nas frequências acima, mantém a correção conhecida com janela fixa. Fica desativada por padrão e também permanece desativada ao carregar presets que não contêm este ajuste. Está disponível somente em Correction; em Minimum e Linear, o controle é desativado, mas o valor selecionado é mantido. Se a resposta ao impulso medida for mais curta que a janela de baixas frequências solicitada, o Room EQ usará a janela de medição mais curta disponível e exibirá um aviso. A correção só será reduzida ou ignorada quando o FIR resultante se aproximar dos limites de tempo ou quando o orçamento de cálculo do projeto do filtro for excedido; o restante do filtro do Room EQ continuará ativo.
...
- Use os botões de opção **Frequência**, **Fase**, **Atraso de grupo** e **Impulso** na parte superior do gráfico para alternar a visualização.
...
- **Atraso de grupo** mostra a frequência em escala logarítmica no eixo horizontal e milissegundos no vertical, indicando quanto cada frequência chega mais tarde que 1 kHz. Cada linha é referenciada ao próprio valor em 1 kHz, de modo que ambas marcam 0 ms nesse ponto e um valor positivo significa que aquela frequência está atrasada em relação a 1 kHz. A linha cinza representa o estado antes da correção e a verde, o resultado calculado após a aplicação do FIR real. A inclinação é obtida entre pontos de exibição vizinhos e depois suavizada apenas por Smoothing, o mesmo passo aplicado à curva de resposta em frequência medida; nenhuma outra suavização é aplicada, portanto um valor menor de Smoothing revela mais detalhes. A faixa vertical se ajusta automaticamente às curvas exibidas. Se a medição não contiver resposta ao impulso, será exibida uma mensagem informando que os dados não estão disponíveis.
- **Impulso** mostra o ponto selecionado ou, quando Reference Point está em Consenso, a forma de onda média alinhada no tempo. O intervalo vai de 2 ms antes do início medido até o maior valor entre 5 ms e Direct Window. A linha cinza representa o sinal antes da correção e a verde, o resultado calculado após a aplicação do FIR real. O início medido é a referência comum de 0 ms e apenas o atraso fixo conhecido do FIR é removido da forma de onda corrigida, de modo que a posição relativa do pico e o pré-ringing permaneçam visíveis. As duas linhas usam a mesma escala de amplitude normalizada. Low-frequency Phase Extension pode analisar uma parte da resposta posterior ao limite desta visualização. Somente para esta exibição, os componentes a partir de 20 kHz são removidos; isso não afeta o filtro de correção nem o processamento de áudio. Se a medição não contiver dados de resposta ao impulso, será exibida uma mensagem informando que eles não estão disponíveis.
