...
- **Reverb Correction** - Ajusta a correção da reverberação da sala medida de 0% a 100% em passos de 1% e atua somente no modo Correction; seus controles ficam desativados em Minimum e Linear, mas os valores são mantidos. No padrão de 0%, a correção de reverberação fica totalmente desativada e o Room EQ se comporta exatamente como sem esse controle. Acima de 0%, o Room EQ analisa a resposta ao longo de Reverb Window e, até Reverb Max Freq, neutraliza a reverberação que continua após o som direto: corrige o excesso de fase da reverberação de forma independente de Phase Correction e adiciona, na mesma faixa, uma correção de magnitude mais fina, que segue Reverb Smoothing e reduz as ondulações em pente e os modos da sala de decaimento lento. Essa correção de magnitude mais fina respeita Max Boost e é dimensionada por Level Correction como o restante da correção automática de magnitude. Abaixo de Phase Low, onde a correção de fase do som direto não chega, atua como uma correção puramente adicional. Com vários pontos de medição, a correção é reduzida automaticamente onde os pontos divergem, o que mantém o resultado padrão natural em toda a área de audição; com um único ponto de medição essa proteção não existe, e a correção vale estritamente para a posição medida. Quando a janela de reverberação ou a faixa de frequências utilizável é pequena demais, ou quando a correção de reverberação sintetizada não cabe com segurança dentro dos limites de tempo do FIR resultante, o Room EQ reduz ou ignora a correção de reverberação e exibe um aviso; o restante do filtro continua ativo.
- **Reverb Window** - Define quanto da resposta medida após o início do som direto, de 20 a 1000 ms, a correção de reverberação analisa e neutraliza. Janelas mais longas alcançam reverberação mais tardia. A janela de reverberação efetiva é limitada automaticamente pelo número de taps do FIR, por Direct Window e pelo comprimento da resposta ao impulso medida após o início; por isso, pode ser mais curta que o valor exibido. Com os 32768 taps padrão a 96 kHz, o limite superior é de cerca de 170 ms. A frequência máxima em que a estrutura fina da reverberação pode ser acompanhada cai em proporção inversa à profundidade da janela: aproximadamente f < 1/(0,0139 × janela efetiva), cerca de 420 Hz com uma janela efetiva de 170,7 ms e cerca de 240 Hz com 300 ms. Se a janela efetiva não ultrapassar Direct Window — seja pelo orçamento de taps, por uma resposta ao impulso medida curta ou por um Reverb Window igual ou inferior a Direct Window —, a correção de reverberação é totalmente ignorada e um aviso é exibido; o restante do filtro continua ativo.
...
- **Reverb Smoothing** - Suavização gaussiana de 0,02 a 1,00 oitava usada pela correção de reverberação tanto na análise de fase quanto na correção de magnitude mais fina. O padrão de 0,05 oct é muito mais fino que o Smoothing principal, para que a correção possa seguir as ondulações estreitas que a reverberação cria na resposta. Valores maiores tornam a correção de reverberação mais ampla e conservadora. Com vários pontos de medição, também define a escala de frequência em que a concordância entre eles é avaliada.
- **Reference Point** - Seleciona a origem do excesso de fase em Correction tanto para a análise do som direto quanto para a da reverberação. **Consenso (todos os pontos)** é o padrão e a opção de contingência: alinha os pontos no tempo, combina o excesso de fase, reduz o peso da fase pouco confiável perto de cancelamentos profundos e diminui a correção onde os pontos divergem. Selecionar um ponto pelo nome usa somente o excesso de fase desse ponto. A correção de magnitude sempre usa todos os pontos. Se o ponto selecionado for excluído, a configuração volta para Consenso.
...
- **Atraso de grupo mínimo** mostra o atraso associado à parte de fase mínima da resposta de magnitude. **Atraso de grupo excedente** mostra separadamente o atraso restante após remover essa parte, facilitando a análise de reflexões e de outros comportamentos temporais que não são de fase mínima. Ambas as vistas usam frequência logarítmica no eixo horizontal e milissegundos no vertical. Cada linha usa como referência o próprio valor em 1 kHz, de modo que ambas marcam 0 ms nesse ponto e um valor positivo indica atraso em relação a 1 kHz. A linha cinza representa o estado antes da correção e a verde, o resultado calculado após a aplicação do FIR real. A inclinação é obtida entre pontos vizinhos e suavizada apenas por Smoothing, como na curva de resposta medida; um valor menor revela mais detalhes. A faixa vertical se ajusta automaticamente. Se a medição não contiver resposta ao impulso, será exibida uma mensagem informando que os dados não estão disponíveis.
