...
### 파라미터
- **Pre-Delay** - 저장되고 표시되는 컨트롤(0에서 50 ms)
  - 현재 구현에서는 이 값이 리버브 처리에 사용되지 않습니다
  - 값을 변경해도 거리감이나 깊이감은 바뀌지 않습니다. 들리는 공간 변화에는 Room Size, Reverb Time, Mix를 사용하세요
...
3. 이펙트 미세 조정
   - 깊이감에는 Room Size와 Reverb Time을 사용하세요. Pre-Delay 컨트롤 값은 현재 처리에 반영되지 않습니다
   - Mix로 최종 밸런스 조정
   - 귀를 믿고 취향에 맞게 조정
