...
2. **음악 폴더 추가**를 선택하고 음악 파일이 들어 있는 폴더를 인덱싱합니다.
...
2.1.0은 새 음악 라이브러리에서 시작합니다. 이전 라이브러리 상태는 이어받지 않으므로 음악 폴더를 다시 추가해 스캔하세요. 이 과정에서 폴더 안의 오디오 파일은 변경되지 않습니다. 카탈로그는 큰 컬렉션을 디스크에서 페이지 단위로 읽도록 설계되었습니다. 대규모 성능 측정은 선택적으로 실행하는 로컬 개발 진단일 뿐이며 commit, release, `verify`, GitHub Actions의 통과 조건이나 일반적인 성능 보장이 아닙니다.
