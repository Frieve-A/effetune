...
- 로컬 폴더, 메타데이터, 플레이리스트를 탐색할 수 있는 음악 라이브러리
...
**설정** 메뉴의 **구성...**을 열면 언어, 시작 시 표시, 시작 시 효과 파이프라인 동작을 선택할 수 있습니다. 시작 시 표시는 **Effect Pipeline (기본값)** 또는 **음악 라이브러리** 중에서 선택할 수 있습니다.

### 음악 라이브러리에서 음악 찾기

1. PC에서는 헤더의 **음악 라이브러리** 버튼, 모바일에서는 **라이브러리** 탭, 데스크톱 앱에서는 **보기 > 음악 라이브러리**에서 엽니다.
2. **음악 폴더 추가**를 선택하고 음악 파일이 들어 있는 폴더를 인덱싱합니다.
3. **트랙**, **앨범**, **아티스트**, **장르**, **폴더**, **최근 추가됨**, **플레이리스트**로 찾아보고, **라이브러리 검색**에서 카탈로그 전체를 검색할 수 있습니다.
