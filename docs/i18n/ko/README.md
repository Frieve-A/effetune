...
**설정** 메뉴의 **구성...**을 열면 언어, 시작 시 표시, 시작 시 효과 파이프라인 동작을 선택할 수 있습니다. 시작 시 표시는 **Effect Pipeline (기본값)** 또는 **음악 라이브러리** 중에서 선택할 수 있습니다. **음악 라이브러리**를 선택한 경우 옆의 목록에서 처음 표시할 항목을 **트랙**, **앨범**, **아티스트**, **장르**, **하위 폴더** 중에서 선택할 수 있습니다.
...
PC용 Chromium 브라우저는 File System Access 폴더 핸들을 영구 저장할 수 있습니다. Safari, Firefox, 모바일 브라우저 및 이 API를 지원하지 않는 환경에서는 선택한 `File` 객체를 현재 페이지 세션 동안만 보관합니다. 새로 고칠 때마다 폴더나 파일을 다시 선택하세요. EffeTune은 정규화된 상대 경로를 사용해 기존 카탈로그에 다시 연결합니다.

EffeTune 2.1.0에서는 카탈로그 스키마 v3를 사용하는 음악 라이브러리가 도입되며, CUE 앨범을 트랙별로 탐색할 수 있습니다. 이전 음악 라이브러리 상태는 이어받지 않으므로 음악 폴더를 다시 추가해 스캔하고, 음악 라이브러리의 플레이리스트도 새로 만들거나 다시 가져오세요. 이전 카탈로그와 오디오 파일은 변경되지 않습니다. 카탈로그는 큰 컬렉션을 디스크에서 페이지 단위로 읽도록 설계되었습니다. 대규모 성능 측정은 선택적으로 실행하는 로컬 개발 진단일 뿐이며 commit, release, `verify`, GitHub Actions의 통과 조건이나 일반적인 성능 보장이 아닙니다.
