...
3.0.0은 새 음악 라이브러리에서 시작합니다. 이전 라이브러리 상태는 이어받지 않으므로 음악 폴더를 다시 추가해 스캔하세요. 이 과정에서 폴더 안의 오디오 파일은 변경되지 않습니다. 새 아키텍처는 100만 곡 이상을 설계 목표로 하지만, 공식 지원은 Phase 0과 전체 런타임 검증을 통과한 뒤 발표합니다.
