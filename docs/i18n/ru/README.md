...
Версия 3.0.0 начинает работу с новой музыкальной библиотекой. Состояние прежней библиотеки не переносится, поэтому заново добавьте и просканируйте папки с музыкой. Аудиофайлы в этих папках не изменяются. Архитектура рассчитана более чем на миллион треков; официальная поддержка будет объявлена только после прохождения Phase 0 и всей квалификации среды выполнения.
