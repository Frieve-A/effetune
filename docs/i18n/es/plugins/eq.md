...
- **Low-frequency Phase Extension** está desactivada de forma predeterminada para conservar el comportamiento habitual de Correction y el sonido de los presets existentes. Actívala solo cuando quieras corregir el exceso de fase por debajo de Phase Low. Las frecuencias más bajas usan ventanas de análisis cada vez más largas y pueden incluir una parte posterior de la respuesta de la sala, por lo que Consenso es el punto de partida más seguro. Compara varias posiciones de escucha y desactiva la extensión si la temporización de los graves se vuelve menos uniforme.
