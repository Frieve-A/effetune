...
- Empieza con **Phase: Minimum**, **Smoothing: 0.17 oct**, **Correction Low: 20 Hz**, **Correction High: 16000 Hz**, **Max Boost: 6 dB** y **Level Correction: 100%**. Compara con el control principal de encendido y apagado del plugin para comprobar que el balance sea más uniforme sin sonar artificialmente delgado o brillante.
...
- **Reverb Correction** - Ajusta la corrección de la reverberación de la sala medida entre el 0% y el 100% en pasos del 1% y solo actúa en el modo Correction; sus controles están desactivados en Minimum y Linear, aunque conservan los valores. Con el 0% predeterminado, la corrección de reverberación está totalmente desactivada y Room EQ se comporta exactamente igual que sin este control. Por encima del 0%, Room EQ analiza la respuesta a lo largo de Reverb Window y, hasta Reverb Max Freq, contrarresta la reverberación que continúa tras el sonido directo: corrige el exceso de fase de la reverberación con independencia de Phase Correction y añade en la misma banda una corrección de magnitud más fina, que sigue a Reverb Smoothing y reduce las ondulaciones en peine y los modos de la sala de caída lenta. Esta corrección de magnitud más fina respeta Max Boost y se escala con Level Correction como el resto de la corrección automática de magnitud. Por debajo de Phase Low, donde la corrección de fase del sonido directo no llega, actúa como una corrección puramente adicional. Con varios puntos de medición, la corrección se reduce automáticamente donde los puntos no coinciden, lo que mantiene el resultado predeterminado natural en toda la zona de escucha; con un único punto de medición esta salvaguarda no está disponible, por lo que la corrección se ajusta estrictamente a la posición medida. Cuando la ventana de reverberación o la banda de frecuencias utilizables son demasiado pequeñas, o cuando la corrección de reverberación sintetizada no cabe con seguridad dentro de los límites temporales del FIR resultante, Room EQ reduce u omite la corrección de reverberación y muestra un aviso; el resto del filtro sigue activo.
- **Reverb Window** - Establece qué parte de la respuesta medida tras el inicio del sonido directo, de 20 a 1000 ms, analiza y contrarresta la corrección de reverberación. Las ventanas más largas alcanzan reverberación más tardía. La ventana de reverberación efectiva se limita automáticamente por el número de taps del FIR, por Direct Window y por la longitud de la respuesta impulsional medida tras su inicio, por lo que puede ser más corta que el valor mostrado; con los 32768 taps predeterminados a 96 kHz el límite superior es de unos 170 ms. La frecuencia máxima a la que puede seguirse la estructura fina de la reverberación cae en proporción inversa a la profundidad de la ventana: aproximadamente f < 1/(0,0139 × ventana efectiva), unos 420 Hz con una ventana efectiva de 170,7 ms y unos 240 Hz con 300 ms. Si la ventana efectiva no supera Direct Window —ya sea por el presupuesto de taps, por una respuesta impulsional medida corta o por un Reverb Window igual o inferior a Direct Window—, la corrección de reverberación se omite por completo y se muestra un aviso; el resto del filtro sigue activo.
...
- **Reverb Smoothing** - Suavizado gaussiano de 0,02 a 1,00 octavas que la corrección de reverberación usa tanto para su análisis de fase como para su corrección de magnitud más fina. El valor predeterminado de 0,05 oct es mucho más fino que el Smoothing principal, para que la corrección pueda seguir las ondulaciones estrechas que la reverberación crea en la respuesta. Los valores altos hacen la corrección de reverberación más amplia y conservadora. Con varios puntos de medición también establece la escala de frecuencia con la que se juzga su coincidencia.
- **Reference Point** - Selecciona la fuente del exceso de fase en Correction tanto para el análisis del sonido directo como para el de la reverberación. **Consenso (todos los puntos)** es el valor predeterminado y de respaldo: alinea temporalmente los puntos, combina su exceso de fase, reduce el peso de la fase poco fiable cerca de cancelaciones profundas y atenúa la corrección donde los puntos no coinciden. Al elegir un punto por su nombre solo se usa el exceso de fase de ese punto. La corrección de magnitud siempre utiliza todos los puntos. Si se elimina el punto seleccionado, el ajuste vuelve a Consenso.
...
- **Retardo de grupo mínimo** muestra el retardo asociado a la parte de fase mínima de la respuesta de magnitud. **Retardo de grupo excedente** muestra por separado el retardo restante tras eliminar esa parte, lo que facilita examinar las reflexiones y otros comportamientos temporales que no son de fase mínima. Ambas vistas usan frecuencia logarítmica en el eje horizontal y milisegundos en el vertical. Cada línea se referencia a su propio valor en 1 kHz, así que ambas marcan 0 ms en ese punto y un valor positivo indica retraso respecto a 1 kHz. La línea gris corresponde a antes de la corrección y la verde, al resultado calculado después de aplicar el FIR real. La pendiente se toma entre puntos contiguos y después se suaviza únicamente con Smoothing, el mismo paso que se aplica a la curva medida; un valor menor muestra más detalle. El rango vertical se ajusta automáticamente. Si la medición no contiene una respuesta al impulso, se muestra un mensaje de datos no disponibles.
