...
- **Tape Artifacts** transforme la musique elle-même. La compression douce, la chaleur ajoutée, les aigus adoucis et l'instabilité de hauteur proviennent tous de la même chaîne d'enregistrement et de lecture : ils réagissent donc ensemble à Speed, Tape, Bias et Input Peak.
...
- **Compression douce du niveau :** plus vous déclarez un Input Peak bas, plus la bande arrondit les crêtes avant de distordre de façon audible ; les passages forts deviennent donc plus denses et plus stables au lieu d'être manifestement écrêtés. À la valeur par défaut de -6,0 dBFS et avec le Bias de référence de 0,0 dB, la bande ne retire que quelques centièmes de décibel aux crêtes d'un programme tenu, et jusqu'à environ 0,8 dB sur les transitoires secs et le bruit. Cette quantité croît régulièrement à mesure que l'on déclare une crête plus basse : 0,4 dB à -15,0 dBFS sur une tonalité à la crête du programme. Tout écart de niveau plus important observé au réglage par défaut vient du changement de timbre et non de la compression, et celui-là joue dans les deux sens selon le programme : une musique très riche en grave peut ressortir environ 1 dB plus fort, un programme très riche en aigu environ 1 dB plus faible.
- **Chaleur :** la saturation est asymétrique et produit donc des harmoniques paires et impaires ; la chaleur s'installe progressivement à mesure qu'Input Peak descend, au lieu d'apparaître d'un coup.
...
- **Un fond vivant :** aux réglages normaux, le souffle et le bruit de modulation qui accompagne la musique font partie du son. Baissez Hiss jusqu'à -110,0 dBFS si vous voulez un fond silencieux.

### Paramètres

- **Speed** (7,5, 15 ou 30 ips) - Choisit la vitesse de défilement. Les vitesses élevées étendent les aigus et déplacent la bosse de grave vers le haut en la réduisant : +1,4 dB à 41 Hz à 7,5 ips, +0,8 dB à 80 Hz à 15 ips et +0,4 dB à 159 Hz à 30 ips. Elles rendent aussi le pleurage et le scintillement plus rapides et moins profonds : Wow/Flutter indique l'écart pondéré à 15 ips et la vitesse le multiplie par 1,5 à 7,5 ips et par 0,75 à 30 ips, si bien que les 0,04 % que la machine de référence publie à 15 ips donnent les 0,06 % et 0,03 % qu'elle publie pour les deux autres. Le bruit ne varie pas dans un seul sens avec la vitesse : le plancher de souffle est le plus élevé à 15 ips et le plus bas à 30 ips, tandis que le bruit de modulation qui accompagne la musique est le plus fort à 7,5 ips. 15 ips est le réglage de studio habituel, 7,5 ips le plus sombre, et 30 ips le plus proche de l'original. Wow/Flutter et Hiss sont donnés tous deux aux 15 ips de référence, et la dernière ligne de l'effet affiche la valeur effective de chacun pour les Speed et Tape choisis.
- **Tape** (Standard ou Master) - Choisit la formulation de la bande. Master a une couche plus épaisse et environ 3 dB de marge supplémentaire avant saturation : elle reste propre plus longtemps et son extrême aigu est un peu plus doux. Aux valeurs élevées d'Input Peak, les deux bandes sont proches en niveau (0,02 dB d'écart au réglage par défaut), mais plus vous baissez Input Peak, plus Master reste forte : 1,2 dB à -24,0 dBFS et 3,8 dB à -36,0 dBFS, justement parce qu'elle sature plus tard ; rétablissez l'égalité de volume avec Output quand vous les comparez.
...
- **Input Peak** (-36,0 à 0,0 dBFS) - Indique à la bande le niveau du programme. Déclarez où votre musique culmine et le modèle place cette crête sur le point de fonctionnement de la bande, qui correspond à une crête de -18,0 dBFS. Ce réglage n'applique aucun gain par lui-même : tant que la bande ne sature pas, le même signal ressort au même niveau quelle que soit sa position. Ce niveau n'est pas exactement l'unité : il en reste à moins de 0,1 dB, un peu au-dessus à 30 ips et un peu au-dessous à 7,5 ips, mais il ne bouge pas avec Input Peak. La valeur par défaut de -6,0 dBFS convient à un master moderne ordinaire ; si vous ignorez où votre programme culmine, laissez-la telle quelle et ajustez à l'oreille. Les valeurs données ici et les réglages ci-dessous supposent une musique dont les crêtes avoisinent -6,0 dBFS ; si votre programme culmine ailleurs, décalez Input Peak de la même différence et les autres valeurs restent valables. Déclarer une crête plus basse que la crête réelle attaque la bande d'autant plus fort, et c'est ainsi que l'on obtient la compression et la chaleur de la bande : une tonalité de 1 kHz à la crête du programme distord de 0,12 % par défaut, de 2 % à -18,0 dBFS et de 6,8 % à -24,0 dBFS, tandis que sur une musique typique les crêtes baissent de 2,7 dB à ce réglage et de 12 dB à -36,0 dBFS. Ces baisses sont la bande qui aplatit les crêtes, non le réglage qui baisse quoi que ce soit : plus la bande est attaquée fort, plus le résultat est faible, et Output est là pour rattraper le volume.
...
2. **Report propre à grande vitesse**
   ...
   - Très proche de l'original : 0,015 % de distorsion aux crêtes, 2,2 dB de moins à 16 kHz, un fond à -93,0 dBFS - ce que devient le réglage Base de -89,5 dBFS à 30 ips sur bande Master, avec Output à 0,0 dB - et 0,053 % de pleurage et scintillement. Déclarer la crête 6 dB plus haut qu'elle ne l'est réellement retire 6 dB à la force avec laquelle la bande est attaquée, et c'est ce qui la garde aussi propre. Utile comme point de repère pour comparer les autres réglages.
...
3. **Chaud et compressé**
   ...
   - Input Peak est déclaré 18 dB sous la crête réelle : la bande est donc attaquée 18 dB plus fort que la normale, soit environ 4 dB au-delà du point où elle atteint 3 % de distorsion de troisième harmonique. Les crêtes baissent de 2,7 dB et le facteur de crête de 2,4 dB sur un programme typique - les crêtes de 1,6 à 4,4 dB selon le programme -, le mixage devient donc plus dense et plus chaud, avec 6,8 % de distorsion sur les crêtes. Output monte légèrement au lieu de descendre, car la compression coûte du niveau sonore ; affinez à l'oreille.
...
4. **Magnétophone domestique à 7,5 ips**
   ...
   - Plus sombre (9,8 dB de moins à 16 kHz, avec une remontée de 1,8 dB à 50 Hz, toutes deux relevées après ses +0,5 dB d'Output), plus bruyant (un fond à -81,5 dBFS, soit le plancher de bande de -82,0 dBFS augmenté du même Output) et moins stable (0,450 % de pleurage et scintillement), avec 1,4 % de distorsion aux crêtes. La polarisation est réglée un peu haut, comme l'est souvent un magnétophone domestique utilisant une bande générique : une machine ordinaire plutôt qu'un appareil de studio.

5. **Défilement usé**
   - Speed : 7,5 ips, Tape : Standard, Bias : -2,0 dB, Input Peak : -28,0 dBFS
   - Wow/Flutter : 0,480 %, Hiss : -77,5 dBFS, Output : +1,5 dB, Mix : 100 %
   - 0,720 % de pleurage et scintillement, 15 % de distorsion sur les crêtes, un fond à -77,5 dBFS - le plancher de bande de -79,0 dBFS augmenté de ses +1,5 dB d'Output - et l'aigu râpeux et en avant d'une machine sous-polarisée. Les crêtes baissent de 5,5 dB sur un programme typique, et jusqu'à 9 dB selon le programme : Output doit donc monter pour rétablir le volume. Un effet lo-fi volontairement dégradé.
