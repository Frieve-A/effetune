...
Room EQ根据EffeTune保存的频率响应测量生成FIR校正滤波器。默认情况下，它会根据共享测量设计一个滤波器，并应用于所有路由到该插件的声道；为单个声道指定不同的测量，即可让该声道拥有自己的滤波器，而其他所有设置在整个实例中仍然保持通用。由插件的标准总线选择器决定处理哪些声道。它会对所选测量的全部测量点取平均、平滑结果，并减小所选校正范围内的偏差。适合用于改善扬声器与房间相互作用在聆听区域造成的稳定峰值或大范围音色失衡。它还支持线性相位幅度校正，以及将最小相位幅度校正与测得的额外相位校正结合起来的混合相位校正：Phase Correction针对直达声，Reverb Correction还可以进一步抵消随后测得的房间混响。默认情况下，两种校正都会保留各测量点共有的成分，并在测量点不一致的部分减弱校正。Room EQ需要WASM DSP引擎；如果不可用，信号将不作改动直接通过。
...
- **Low-frequency Phase Extension** - 在Phase Low以下使用频率越低、长度越长的频率相关分析窗，将测得的额外相位校正从Phase Low向Correction Low扩展。Phase Low及更高频率使用固定分析窗。此设置默认关闭。它只能在Correction中使用；在Minimum和Linear中，控件会停用，但所选值仍会保留。如果测得的脉冲响应比所需的低频时间窗短，Room EQ会使用可用的较短测量时间窗并显示警告。只有在实际生成的FIR接近其时间限制或滤波器设计超出计算预算时，才会减小或跳过校正；Room EQ滤波器的其余部分仍然有效。该扩展只有在Phase Correction大于0%时才起作用；设为0%时，即使同时使用Reverb Correction，它也保持不活动。
...
- **Reverb Correction** - 以1%为步进，在0%到100%之间调节对测得房间混响的校正，并且仅在Correction模式下生效；在Minimum和Linear中控件会被禁用，但数值仍会保留。默认的0%会完全关闭混响校正，Room EQ的行为与没有此控件时完全相同。大于0%时，Room EQ会在Reverb Window范围内分析响应，并在Reverb Max Freq以下抵消直达声之后持续的混响：它独立于Phase Correction校正混响的额外相位，并在同一频段内加入跟随Reverb Smoothing的更精细幅度校正，以减小梳状波纹和衰减缓慢的房间模式。这一精细幅度校正受Max Boost限制，并像其余自动幅度校正一样按Level Correction缩放。在直达声相位校正达不到的Phase Low以下，它作为纯粹的附加校正起作用。有多个测量点时，会在测量点不一致的部分自动减弱校正，使默认结果在整个聆听区域保持自然；只有一个测量点时没有这一保护，校正严格针对测得的位置。可用的混响时间窗或频段过小，或者合成的混响校正无法安全地纳入实际生成的FIR的时间限制内时，Room EQ会减小或跳过混响校正并显示警告；滤波器的其余部分仍然有效。
- **Reverb Window** - 设置混响校正分析并抵消直达声起点之后多长的测量响应，范围为20到1000 ms。窗口越长，能处理的混响越晚。实际生效的混响时间窗会被FIR taps数量、Direct Window以及测得脉冲响应在起点之后的长度自动限制，因此可能比显示值短；默认32768 taps、96 kHz时上限约为170 ms。能够跟踪混响精细结构的最高频率与时间窗深度成反比：大约为f < 1/(0.0139 × 有效时间窗)，有效170.7 ms时约为420 Hz，300 ms时约为240 Hz。如果有效时间窗没有超过Direct Window——无论是由于taps预算、测得脉冲响应过短，还是Reverb Window被设为不高于Direct Window——混响校正会被完全跳过并显示警告；滤波器的其余部分仍然有效。
...
- **Reverb Smoothing** - 混响校正在相位分析和精细幅度校正中都使用的0.02到1.00倍频程高斯平滑。默认的0.05 oct比主Smoothing精细得多，使校正能够跟随混响在响应中产生的狭窄波纹。数值越大，混响校正越宽缓、保守。有多个测量点时，它还决定评估各点一致性的频率尺度。
- **Reference Point** - 选择Correction在直达声与混响两种分析中所使用的额外相位来源。**一致成分（所有测量点）** 是默认值和回退选项：它会在时间上对齐各测量点、合并额外相位、降低深度响应凹陷附近不可靠相位的权重，并在测量点之间相位不一致时减弱校正。选择具名测量点后，只使用该点的额外相位。幅度校正始终使用全部测量点。如果所选测量点之后被删除，设置会回到一致成分。
...
- **最小群延迟**显示幅度响应中最小相位部分对应的延迟。**超额群延迟**则单独显示扣除该部分后剩余的延迟，便于查看反射和其他非最小相位时间变化。两种视图的横轴均为对数频率，纵轴均为毫秒。每条线都以自身在1kHz处的值为基准，因此两条线在1kHz处均为0 ms，正值表示相对1kHz有延迟。灰线表示校正前，绿线表示应用实际FIR后计算出的结果。斜率取自相邻显示点之间，并且只应用与测得频率响应相同的一次Smoothing；Smoothing越小，显示的细节越多。纵轴范围会自动调整。如果测量不含脉冲响应，界面会显示不可用提示。
