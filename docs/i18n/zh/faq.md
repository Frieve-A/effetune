...
| 支持哪些音乐文件格式？ | 取决于浏览器和操作系统的音频解码能力。通常 MP3、WAV、AAC/M4A 在多数环境中较容易播放；FLAC、OGG/Vorbis、Opus/WebM 等格式的支持因环境而异。无法播放时，请尝试 MP3、AAC/M4A 或 WAV。 |
| 可以一次打开多个音乐文件吗？ | 可以。点击 **打开音乐文件** 后，在设备自带的文件选择界面中选择多个文件再打开，它们会作为播放列表载入。是否支持多选或一次选中文件夹内所有文件，取决于设备、浏览器和文件选择界面。 |
| 音乐库有什么作用？ | 它会为选中的音乐文件夹建立索引，让您按曲目、专辑、艺人或流派浏览和搜索，并在 EffeTune 中播放结果。库元数据和播放列表保存在应用内，不会写入音频文件。 |
...
| 为什么 **采样率：** 或 **输出通道：** 不是指定值？ | 浏览器或设备可能会对不支持的值进行调整或直接忽略。EffeTune 会按实际生效的值运行。 |
