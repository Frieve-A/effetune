...
| “静音阈值”和“在以下时间后停止音频输入”分别控制什么？ | **静音阈值**（-90/-80/-70/-60 dBFS）决定输入和输出的测量功率低于何值时被视为静音；数值越低，越不容易把微弱声音判为静音。在 **最大限度节能** 中，**在以下时间后停止音频输入**（1/5/15 分钟或 **永不**）只控制麦克风等输入的释放。它与无路由音频图进入 Suspended 的较短延迟相互独立，因此可能在保留输入的同时暂停音频图。 |
