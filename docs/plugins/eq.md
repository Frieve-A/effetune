...
- **Reverb Correction** - Scales correction of the measured room reverberation from 0% to 100% in 1% steps and affects only the Correction mode; its controls are disabled in Minimum and Linear while the values are retained. At the default 0%, reverb correction is fully off and Room EQ behaves exactly as without this control. Above 0%, Room EQ analyzes the response through Reverb Window and, up to Reverb Max Freq, counteracts reverberation that continues after the direct sound: it corrects the reverberation's excess phase independently of Phase Correction, and adds a finer magnitude correction in the same band, following Reverb Smoothing, that reduces comb-like ripple and slowly decaying room modes. This finer magnitude correction stays within Max Boost and is scaled by Level Correction like the rest of the automatic magnitude correction. Below Phase Low, where direct-sound phase correction does not reach, it acts as a purely additional correction. With multiple measurement points, correction is reduced automatically where the points disagree, which keeps the default result natural across the listening area; with a single measurement point this safeguard is unavailable, so the correction applies strictly to the measured position. When the usable reverb window or frequency band is too small, or when the synthesized reverb correction does not fit safely within the time limits of the realized FIR, Room EQ reduces or skips the reverb correction and shows a warning; the rest of the filter remains active.
- **Reverb Window** - Sets how much of the measured response after the direct-sound onset, from 20 to 1000 ms, reverb correction analyzes and counteracts. Longer windows reach later reverberation. The effective reverb window is clamped automatically by the FIR tap count, Direct Window, and the measured impulse response's length after its onset, so it can be shorter than the displayed value; with the default 32768 taps at 96 kHz the upper limit is about 170 ms. The highest frequency at which the fine reverb structure can be followed falls in inverse proportion to the window depth — roughly f < 1/(0.0139 × effective window), about 420 Hz at an effective 170.7 ms and about 240 Hz at 300 ms. If the effective window does not exceed Direct Window — whether from the tap budget, a short measured impulse response, or a Reverb Window set at or below Direct Window — reverb correction is skipped entirely and a warning is shown; the rest of the filter remains active.
...
- **Reverb Smoothing** - Gaussian frequency smoothing from 0.02 to 1.00 octaves used by reverb correction for both its phase analysis and its finer magnitude correction. The default 0.05 oct is much finer than the main Smoothing so the correction can follow the narrow ripple that reverberation creates in the response. Higher values make reverb correction broader and more conservative. With multiple measurement points it also sets the frequency scale on which their agreement is judged.
...
- **Min Group Delay** shows the delay implied by the minimum-phase part of the magnitude response. **Excess Group Delay** separately shows the remaining delay after that minimum-phase part is removed, making reflections and other non-minimum-phase timing easier to inspect. Both views use a logarithmic frequency axis and a vertical axis in milliseconds. Each line is referenced to its own value at 1 kHz, so both read 0 ms there and a positive value means that frequency is delayed relative to 1 kHz. The gray line is before correction and the green line is the calculated result after the actual FIR. The slope is read between adjacent display points and then smoothed by Smoothing, the same single pass applied to the measured frequency curve; no other smoothing is applied, so a lower Smoothing setting shows finer detail. The vertical range is scaled automatically to the displayed curves. A measurement without impulse-response data shows an unavailable message instead.
