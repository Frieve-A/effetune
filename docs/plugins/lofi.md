...
- **Tape Artifacts** changes the music itself. The gentle compression, the added warmth, the softened treble, and the pitch wobble all come from the same modeled record-and-reproduce chain, so they respond together to Speed, Tape, Bias, and Input Peak.
...
- **Gentle level compression:** the lower you declare Input Peak, the more the tape rounds the peaks off before it audibly distorts, so loud passages become denser and steadier rather than obviously clipped. At the default -6.0 dBFS and the reference 0.0 dB Bias the tape takes only a few hundredths of a decibel off the peaks of sustained material, and up to about 0.8 dB off sharp transients and noise. The amount it takes off grows smoothly as you declare a lower peak, reaching 0.4 dB at -15.0 dBFS on a tone at the material's peak. Any larger level change you see at the default comes from the tone shaping and not from the compression, and that one goes either way depending on the material: bass-heavy music can come out about 1 dB louder, and material with a lot of top about 1 dB quieter.
- **Warmth:** the saturation is asymmetric, so it produces both even and odd harmonics, and the warmth grows gradually as Input Peak comes down instead of appearing suddenly.
...
- **A living background:** hiss, plus modulation noise that rides on the music itself, is part of the sound at normal settings. Take Hiss all the way down to -110.0 dBFS when you want a silent background.

### Parameters

- **Speed** (7.5, 15, or 30 ips) - Selects the tape speed. Faster speeds extend the treble and move the low-frequency head bump higher in frequency while making it smaller: +1.4 dB at 41 Hz at 7.5 ips, +0.8 dB at 80 Hz at 15 ips, and +0.4 dB at 159 Hz at 30 ips. They also make the wow and flutter faster and shallower: Wow/Flutter states the weighted deviation at 15 ips and the speed multiplies it by 1.5 at 7.5 ips and by 0.75 at 30 ips, so the 0.04% the reference machine publishes at 15 ips gives the 0.06% and 0.03% it publishes at the other two. The noise does not move in one direction with the speed: the hiss floor is highest at 15 ips and lowest at 30 ips, while the modulation noise that rides on the music is strongest at 7.5 ips. 15 ips is the usual studio setting, 7.5 ips is the darkest, and 30 ips is the closest to the original. Wow/Flutter and Hiss are both stated at the reference 15 ips, and the last line of the effect shows what each of them comes to at the Speed and Tape you have selected.
- **Tape** (Standard or Master) - Selects the tape formulation. Master has a thicker coating and about 3 dB more headroom before the tape saturates, so it stays clean longer and has a slightly softer top end. At high Input Peak settings the two are close in level (0.02 dB apart at the default), but the lower you set Input Peak the louder Master stays: 1.2 dB at -24.0 dBFS and 3.8 dB at -36.0 dBFS, precisely because it saturates later, so re-match the loudness with Output when you compare them.
...
- **Input Peak** (-36.0 to 0.0 dBFS) - Tells the tape how loud the material is. State where your music peaks and the model places that peak on the tape's operating point, which corresponds to a peak of -18.0 dBFS. The control adds no gain of its own: as long as the tape is not saturating, the same signal comes out at the same level whatever Input Peak is set to. That level is not exactly unity - it sits within 0.1 dB of it, a little high at 30 ips and a little low at 7.5 ips - but it does not move with Input Peak. The default -6.0 dBFS suits a normal modern master; if you do not know where your material peaks, leave it there and adjust by ear. The figures here and the settings below assume music that peaks around -6.0 dBFS; if your material peaks somewhere else, move Input Peak by the same difference and the rest of the values still hold. Declaring a lower peak than the material really has drives the tape harder by that same difference, and that is how you get tape compression and warmth out of it: a 1 kHz tone at the material's peak distorts 0.12% at the default, 2% at -18.0 dBFS and 6.8% at -24.0 dBFS, while on typical music the peaks come down by 2.7 dB at that setting and by 12 dB at -36.0 dBFS. Those drops are the tape flattening the peaks, not the control turning anything down, so the harder you drive the tape the quieter the result gets, and Output is there to bring the loudness back.
...
- **Hiss** (-110.0 to -60.0 dBFS) - Sets the level of the tape hiss and the modulation noise together, as the A-weighted hiss floor at 15 ips on Standard tape. -110.0 dBFS switches both off completely. The default -83.5 dBFS is the bias noise the manufacturer publishes for that tape at that speed, referred to the operating level; the other speeds and Master tape differ from it by the amounts the datasheet gives, so at that default the six combinations run from -83.0 to -87.0 dBFS, and the whole set moves with the control. All of these are the floor on the tape, ahead of Output, so a meter placed after Output reads them lifted by whatever Output is set to. That floor is what you hear in the gaps; while the music plays what this control mostly adds is the modulation noise riding on the signal, about 57 dB below a steady tone on Standard tape at 15 ips, and a few decibels either side of that at the other Speed and Tape settings and on real material. Higher values make the background more obvious.
- **Output** (-24.0 to +24.0 dB) - Adjusts the level after the whole chain. Use it to match loudness when you compare with bypass, or to bring back the loudness a low Input Peak setting has cost.
- **Mix** (0 to 100%) - Blends the tape signal with the original. 100% is full tape playback. The dry signal is delay-aligned with the tape path, so the midrange blends cleanly - 1 kHz stays within 0.1 dB of unity at every Mix setting and Speed at the reference 0.0 dB Bias, and within 0.5 dB anywhere in the Bias range, with Input Peak matched to the material - but the top octave does not, because dry and tape no longer share the same phase up there and partly cancel. At 50% the level at 16 kHz comes out 1.7 dB down on a 44.1 kHz host, 2.1 dB on 48 kHz, 4.6 dB on 96 kHz, and 5.7 dB on 192 kHz, and on a 96 or 192 kHz host the darkest point of the control is not 100% but around 70%. On a 44.1 kHz host it only gets darker as you turn it up, and on a 48 kHz one the darkest point is 89%, 0.06 dB below 100%, so on both the middle of the control is brighter at the very top than 100% is. At 0% the input passes through completely unchanged and the effect adds no latency; at any other setting it adds 5.26 ms on a 44.1 kHz host, falling to 5.06 ms on a 192 kHz one.
...
1. **Studio Master Tape (default)**
   ...
   - The everyday tape sound, and the plugin's own default: the top softened by 3.5 dB at 16 kHz, a 0.8 dB lift around 80 Hz, 0.12% distortion at the peaks, a hiss floor at -83.5 dBFS, and 0.160% wow and flutter, audible on held notes and not on transients.
...
2. **Clean High-Speed Transfer**
   ...
   - Very close to the original: 0.015% distortion at the peaks, 2.2 dB down at 16 kHz, a -93.0 dBFS background - what the -89.5 dBFS Base setting becomes at 30 ips on Master tape, with Output at 0.0 dB - and 0.053% wow and flutter. Declaring the peak 6 dB higher than it really is takes 6 dB off how hard the tape is hit, which is what keeps it this clean. Useful as a reference point when comparing the other settings.
...
3. **Warm and Compressed**
   ...
   - Input Peak is declared 18 dB below the real peak, so the tape is hit 18 dB harder than nominal, about 4 dB past the point where it reaches 3% third-harmonic distortion. Peaks fall by 2.7 dB and the crest factor by 2.4 dB on typical material - the peaks anywhere from 1.6 to 4.4 dB depending on the material - so the mix becomes denser and warmer, with 6.8% distortion at the peaks. Output goes slightly up, not down, because the compression costs loudness; fine-tune it by ear.
...
4. **Home Deck at 7.5 ips**
   ...
   - Darker (9.8 dB down at 16 kHz, with a 1.8 dB lift at 50 Hz, both measured after its +0.5 dB of Output) and noisier (a -81.5 dBFS background, which is the tape's own -82.0 dBFS floor plus that same Output), and less steady (0.450% wow and flutter), with 1.4% distortion at the peaks. The bias is set a little high, the way a domestic deck running generic tape usually is: an ordinary machine rather than a studio one.

5. **Worn Transport**
   - Speed: 7.5 ips, Tape: Standard, Bias: -2.0 dB, Input Peak: -28.0 dBFS
   - Wow/Flutter: 0.480%, Hiss: -77.5 dBFS, Output: +1.5 dB, Mix: 100%
   - 0.720% wow and flutter, 15% distortion at the peaks, and a -77.5 dBFS background - the tape's own -79.0 dBFS floor plus its +1.5 dB of Output - with the gritty, forward top of an under-biased machine. Peaks fall 5.5 dB on typical material, and up to 9 dB depending on the material, so Output has to come up to bring the loudness back. An intentionally degraded lo-fi effect.
