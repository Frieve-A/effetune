...
extern "C" const effetune::KernelDescriptor *et_kernel_descriptor_GroupDelayEqPlugin() noexcept;
...
void check(bool condition, const char *expression, int line) noexcept {
  if (condition)
    return;
  std::fprintf(stderr, "group_delay_eq/native_test.cpp:%d: check failed: %s\n", line, expression);
  ++failures;
}
...
struct Harness final {
  ...
  const effetune::KernelDescriptor *descriptor = et_kernel_descriptor_GroupDelayEqPlugin();
