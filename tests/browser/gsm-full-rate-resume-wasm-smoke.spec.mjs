...
const FIXTURE_PATH = '/tests/browser/gsm-full-rate-resume-wasm-smoke.fixture.html';

export async function runGsmFullRateResumeWasmBrowserSmoke({ browser, baseURL }) {
  ...
  try {
    ...
    const result = await page.evaluate(() => window.__gsmFullRateResumeWasmSmoke.run());
    assert.equal(result.pluginType, 'GSMFullRateSimulatorPlugin');
    ...
    // No stale audio survives: the instance that kept its RPE-LTP predictor state does diverge from
    // the fresh one, which proves the reset above is a real state change and not a silent no-op.
