...
test('production schemas generate a fresh private automation catalog for all 83 effects', async () => {
  const specs = loadParamSpecs();
  assert.equal(specs.length, 83);
  ...
  assert.equal(Object.keys(catalog.effects).length, 83);
  ...
  assert.equal(Object.keys(generated.DSP_AUTOMATION_CATALOG).length, 83);
  assert.equal(
