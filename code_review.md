...
- Before pushing, run `npm audit --audit-level=moderate`. For dependency metadata changes, also run the CI-equivalent `npm ci --ignore-scripts`, `npm audit signatures`, and `npm run verify`; reject unresolved peer conflicts instead of forcing installation. Routine non-security Dependabot updates are not a release gate, but known security findings are.
